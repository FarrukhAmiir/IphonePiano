...
using System;
using System.Linq;
using TDEditor.iOS.Xcode.PBX;
using UnityEngine;

namespace TDEditor.iOS.Xcode
{
    ...
    public class PBXProject
    {
        ...
        */
		...
		public List<string> GetBuildProperties(string targetGuid, string name) {
			foreach (string guid in configs[GetConfigListForTarget(targetGuid)].buildConfigs) {
				var res = buildConfigs [guid].GetPropertyValues (name);
        ...
        public void EmbedFramework(string target, string framework, bool weak = false, bool codeSignOnCopy = true,
            bool removeHeadersOnCopy = true)
        {
            m_Data.EmbedFramework(target, framework, weak, codeSignOnCopy, removeHeadersOnCopy);
        }
    }
