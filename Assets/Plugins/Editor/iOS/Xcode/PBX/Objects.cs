...
namespace TDEditor.iOS.Xcode.PBX
{
    ...
    internal class PBXBuildFileData : PBXObjectData
    {
        ...
        public override void UpdateProps()
        {
            ...
            if (weak || codeSignOnCopy || removeHeadersOnCopy)
            {
                if (settings == null)
                    settings = m_Properties.CreateDict("settings");
                ...
                if (codeSignOnCopy)
                    CheckParameterInAtributes(attrs, "CodeSignOnCopy");
                if (removeHeadersOnCopy)
                    CheckParameterInAtributes(attrs, "RemoveHeadersOnCopy");
                if (weak)
                    CheckParameterInAtributes(attrs, "Weak");
            }
            else
            {
        ...
        private void CheckParameterInAtributes(PBXElementArray attrs, string parameter)
        {
            var exists = false;
            foreach (var value in attrs.values)
            {
                if (value is PBXElementString && value.AsString() == parameter)
                    exists = true;
            }
            if (!exists)
                attrs.AddString(parameter);
        }
        ...
        public override void UpdateVars()
        {
            ...
            compileFlags = null;
            removeHeadersOnCopy = codeSignOnCopy = weak = false;
            ...
            if (m_Properties.Contains("settings"))
            {
                ...
                if (dict.Contains("ATTRIBUTES"))
                {
                    ...
                    weak = attrs.values.Exists(v => v.AsString() == "Weak");
                    codeSignOnCopy = attrs.values.Exists(v => v.AsString() == "CodeSignOnCopy");
                    removeHeadersOnCopy = attrs.values.Exists(v => v.AsString() == "RemoveHeadersOnCopy");
                }
    ...
    internal class XCBuildConfigurationData : PBXObjectData
    {
		...
		public List<string> GetPropertyValues(string name) {
			if(entries.ContainsKey(name) && entries [name] != null)
				return entries [name].val;
			return null;
		}
